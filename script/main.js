...
// 实际上是已知所在面、X坐标、Y坐标，以及点坐标系将要旋转的角度
// 得出旋转坐标系  之后的点坐标
...
this.bodyLoc[6] = new CreatePoint(0, 50, 300, 0);
 ...
 function createFrog(){
  ...
  canvasContext[faceLoc].fillStyle = "#ee0000";
  ...
  for (var i = 0; i < snake.bodyLength; i++) {
   ...
   canvasContext[faceLoc].fillStyle = "#dc143c";
 ...
 function timer(){
  if (gameStatus != 1) {  //判断游戏是否进行中
    ...
    snake.run();  //蛇移动一步
  ableChangeDirect = true;  //蛇移动完后解锁移动方向,防止在移动中途换方向

    if (snake.bodyLoc[0].equal(frogLoc)) {  //判断是否吃到青蛙
      snake.eatFrog();
    }

   if (snake.bodyLoc[0].face != snake.bodyLoc[1].face) { //已经换面了，旋转动画
   ...
   if (!(gameStatus == 1 && ableChangeDirect)) {
    ...
    case 37:  //key left
      ...
      event.preventDefault(); 
        break;
      case 74:  //key J，测试用
        console.log("方向： " + snake.runDirect + " , 画布当前面： " + canvasFrontFace + " ,画布当前面旋转度：  " + canvasFrontFaceDir)
      return;
    case 80:  //key P，暂停
      alert("pause");
        return;
